...
export const EditItemModal = ({onClick, editItemModal, itemId})  =>
{
        const advertState = appStore((state) => state)
        const navigate = useNavigate();

        const [theProductId, setTheProductId] = useState(itemId)
        ...
        const updateFaqData = () => 
        {
                ...
                .then((res) => 
                {
                        // return onClick(itemId*Math.random())
                })
                .catch((err) => 
                {
                        console.log(err)
        ...
        return (
                <Modal onClick={onClick} isOpen={editItemModal} wrapperWidth={800} margin={'100px auto 0px auto'}>
                                ...
                                <div className='col-span-12 pb-2 overflow-auto justify-center h-fit py-2 item-center'>
                                        <>                                                
                                                <div className="p-1 mt-1">
                                                        <h1 className='font-bold text-lg mb-10'>Edit Item</h1>
                                                        <div className="w-full d-flex-row md:flex mt-1 gap-5 mb-5">
                                                                {/* <span className="font-bold text-sm">Title</span> */}
                                                                <input onBlur={(e) => {
                                                                        
                                                                }} type="text" id="course" defaultValue={'course'}  name="course" 
                                                                placeholder='Great' 
