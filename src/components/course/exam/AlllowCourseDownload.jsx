...
export const AlllowCourseDownload = ({onClick, openCourseDownloadStatus, course})  =>
{
        const [loading, setIsLoading] = useState(false)
        // alert(course.id)

        const AllowCourseDownload = () => 
        {
                setIsLoading(true)
                // RemoveCourse(data)
                // .then((res) => 
        ...
        return (
                <Modal onClick={onClick} isOpen={openCourseDownloadStatus} wrapperWidth={800} margin={'100px auto 0px auto'}>
                        <div className='col-span-12 pt-1 pb-5 overflow-y-auto xm:overflow-y-scroll d-flex justify-center item-center'>
                                ...
                                <div className="items-center gap-5 mt-2 sm:flex flex justify-between mb-2 mx-2 mt-5">
                                        <button  
                                                ...
                                                onClick={() => {
                                                        onClick(!openCourseDownloadStatus)
                                                }}
                                        >
                                                        Cancel
                                        </button>
                                        <button
                                                ...
                                                onClick={AllowCourseDownload}
                                                >                                                
                                                {       loading ? ( <BeatLoader size={9} color="#fff" />) : ( "Allow" )          }
                                        </button>
                                </div>
                        </div>
                </Modal>  
