...
import { ExamTheoryDurration } from '@/apis/backend/course';


export const ExamTheoryDuration = ({onClick, openExamStatus, exam})  =>
{
        const [loading, setIsLoading] = useState(false)
        const [objectiveDuration, setExamTheoryDuration] = useState(false)

        const EnterExamTheoryDuration = () => 
        {
                setIsLoading(true)
                const data = { id: exam?.id, exam_theory_duration: objectiveDuration }
                ExamTheoryDurration(data)
                .then((res) => 
                {
                        setIsLoading(false)
                        onClick()
                                ...
                                >
                                        ...
                                        <div className="w-1/2 md:flex gap-5 mb-5">
                                                <input onChange={(e) => {
                                                        ...
                                                        type="number" id="objDuration" defaultValue={''}  name="objDuration" 
