...
export default function TestQuestionaireTheory({ academicSession })
{   
    ...
    const { data, isLoading, isRefetching, refetch } = useQuery([`test-questionaires-theory`], () => AllTestQuestionairesTheory(currentSession), { cacheTime: 0 })
  ...
  return ( 
        <>
            {
                !isLoading && <div className="w-full justify-between p-3 flex -mb-3 items-center rounded-lg">                        
                    <div className="p-3 mt-5">
                        <h1 className="font-bold text-xl text-blue-500">Test Theory Questionaires (Theory)</h1>
                    </div>
                    {/* { !isLoading && (data?.length < 1) && <div className="font-bold px-3 py-2 bg-green-900 text-white rounded-md cursor-pointer" onClick={() => setOpenQuestionaireTheory(true)}>Add Theory Question</div>} */}
            ...
            <div className="w-full p-1 mb-10 rounded-lg" style={{ marginBottom: '100px' }}> 
                ...
                <div className='grid grid-cols-12 gap-5 py-2 px-3 mb-5'>
                {
                    
                    !isLoading && !isRefetching && (data?.length > 0) && data?.map((data, index) => <Folder type={"test-theory"} toEdit={"editTestTheory"} toDelete={"deleteTestTheory"} id={data?.id} titles={data?.name} description={data?.description} figures={100} icons={'comment'} onClick={(e) => {
                            refetch()
                        }
                    } />)
                }
