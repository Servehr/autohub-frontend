...
import ResloveIssue from "./ResloveIssue";
import { isPaidAndStudentSummary } from "@/apis/user";


export default function StartObjTest() 
{

  const advertState = appStore((state) => state)
  const { data, isLoading, refetch, isRefetching} = useQuery([`check-if-user-has-paid`], () => isPaidAndStudentSummary('test-objective'), { refetchOnWindowFocus: true, cacheTime: 0 }, { refetchOnWindowFocus: true, cacheTime: 0 })
        ...
        <div
          ...
          >
                ...
                {  !isLoading && data?.data && (data?.data?.payment_status === "paid") && (data?.message === 'valid') && (data?.plus === "open") && 
                    <>
                        {/* <StartPage course={data?.plus} option={data?.message} />  */}
                        <StartPage /> 
                    </>
                }
