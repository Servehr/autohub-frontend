...
import { SubmitExamTheory, TestQuestions } from "@/apis/backend/course";
import { appStore } from "@/state/appState";


export default function UserTakeExamTheory() 
{
  ...
  const [choosen, setChoosen] = useState(advertState.setSelectedExamTheoryOption())
  ...
  const deselectAll = () => 
  {
      setChoosen(advertState.setSelectedExamTheoryOption())
  ...
  {
      const checkIfPresent = advertState.setSelectedExamTheoryOption().findIndex(x => {
  ...
  const isSelected = (id) => 
  {
      const answeredOption = advertState.setSelectedExamTheoryOption()
  ...
  {
      let checkedValue;
      const theChoosen = advertState.setSelectedExamTheoryOption()
              ...
              }
              {
                !isLoading && isRefetching && (data?.data?.length === 0) && <div className="col-span-12 h-[500px] flex justify-center items-center border border-3 border-shadow border-green-200 bg-[#f5fbf7]" style={{ marginTop: '30px', paddingTop: '20px' }}>
                    <h1 className="font-bold">
                        Theory Objective Question Not Yet Prepared
                    </h1>
                </div>
              }
