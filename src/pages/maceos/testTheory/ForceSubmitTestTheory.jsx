...
import { AllCourse, SubmitTestTheory } from "@/apis/backend/course";
...
export default function ForceSubmitTestTheory() 
{
  ...
  useEffect(() => 
  {
      if(advertState.getSelectedTestTheoryOption().length === 0)
      {
          ...
          const systemAnswer = { userSubmitted : 'yes', answers: advertState.getSelectedTestTheoryOption() }
          advertState.setDefaultTestTheoryAnswer(systemAnswer)
      }
  }, [])

  const SubmitTestTheoryy = () => 
  {
    setIsSubmitting(true)
    SubmitTestTheory(advertState.getDefaultTestTheoryAnswer())
       .then((res) => {
           if(res === "submitted")
           {
               advertState.setForceTestTheory('no')
