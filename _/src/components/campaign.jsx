        ...
        { !isLoading &&
          <>        
            <div className="md:hidden lg:hidden xs:block sm:block p-1 py-7 -mt-5">
                    <div className="px-1 font-bold text-xl text-green-600 mb-2">Categories</div>
                    <ul className="p-3 gap-3 mt-3 grid grid-cols-12 flex-col md:hidden">
                        {!isLoading && data?.data?.map((category, idx) => (
                          <li key={idx} className="col-span-4 box-shadow">
                            <Link
                              to={category.link}
                              className="h-full gap-2 flex flex-col items-center justify-between rounded cursor-pointer duration-300 transition flex hover:shadow-inner rounded-lg"
                              role="button"
                            >
                              <div className="flex justify-center gap-3 items-center bg-brandGray w-full h-[72px] rounded-lg hover:bg-green-300">
